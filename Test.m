    ...
    if dataSets(setCounter).isdir ==1 && strcmp(dataSets(setCounter).name,'.') ==0 && strcmp(dataSets(setCounter).name,'..') == 0 &&strcmp(dataSets(setCounter).name,'Set1 - aggiornato')==0 ...
        ...
        for sourceCounter = 1:size(dataSources,1)
           ...
           %% store the result in excel
            ...
            dataRecord = {strcat(directory,dataSets(setCounter).name,'\',dataSources(sourceCounter).name),desc(2),bestCost,costs1,costs2};
            xlswrite('result\twoPhaseResult.xls',dataRecord,'version3',strcat('A',num2str(counter)));
            counter = counter + 1;
            end
        end
    end
