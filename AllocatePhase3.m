...
%% 初始状态
maxSolutionLen = size(nodeList,1)-depotNum + 1 + vNum; %默认每个depot有四辆车
...
if isempty(routes)
    ...
    for i = depotNum+1:size(nodeList,1)
        ...
        while successFlag == 0
            depotSeq = SortDepotInDistance(depotNum,distances,i);
            [successFlag,customer,routes] = AllocateCusToDepot(depotSeq,capacities,distances,demands,routes,maxSolutionLen,customer);
        end
    end
...
for depotIndex = 1:depotNum
    ...
    if totalDemand + demands(customer) <= capacities(depot)
        ...
        if totalDemand -demands(curRoute(replace)) + demands(customer) <= capacities(depot)
